...
public class ReadingType3Activity extends AppCompatActivity{
    ...
    protected void onCreate(Bundle savedInstanceState) {
        ...
        setContentView(R.layout.activity_reading_type3);
        ImmersionBar.with(ReadingType3Activity.this).statusBarColor(R.color.main).init();
        ...
        position = getIntent().getIntExtra("position", 30);
        loadData();
    }
    ...
    private void loadImage(String path, ImageView imageView) {
        ...
        downloadUrl.addOnCompleteListener(new OnCompleteListener<Uri>() {
            ...
            public void onComplete(@NonNull Task<Uri> task) {
                if (task.isComplete()){
                    Glide.with(ReadingType3Activity.this).load(task.getResult().toString()).into(imageView);
                }else {
                    Log.e("---->","fail------");
                }
            }
    ...
    protected void onDestroy() {
        super.onDestroy();
        EventBus.getDefault().post(new PositionEvent(2,position));
    ...
    private void loadData() {
        ...
        String aPath = path + "BandA_" + dataReadings.get(position-30).getVersion() + "RdPIC/" + toUP(dataReadings.get(position-30).getTopicimage());
        ...
        imageView1.setText(dataReadings.get(position-30).getA());
        imageView2.setText(dataReadings.get(position-30).getB());
        imageView3.setText(dataReadings.get(position-30).getC());

        answer.setText(TextUtils.isEmpty(Util.strings.get(position))?"":"Selected: "+Util.strings.get(position));

        question.setText(dataReadings.get(position-30).getQuestion());
    }
