...
public class ReadingType5Activity extends AppCompatActivity{
    ...
    protected void onCreate(Bundle savedInstanceState) {
        ...
        setContentView(R.layout.activity_reading_type5);
        ImmersionBar.with(ReadingType5Activity.this).statusBarColor(R.color.main).init();
        ...
        position = getIntent().getIntExtra("position", 45);
        loadData();
    }
    ...
    private void loadImage(String path, ImageView imageView) {
        ...
        downloadUrl.addOnCompleteListener(new OnCompleteListener<Uri>() {
            ...
            public void onComplete(@NonNull Task<Uri> task) {
                if (task.isComplete()){
                    Glide.with(ReadingType5Activity.this).load(task.getResult().toString()).into(imageView);
                }else {
                    Log.e("---->","fail------");
                }
            }
    ...
    protected void onDestroy() {
        super.onDestroy();
        EventBus.getDefault().post(new PositionEvent(4,position));
    ...
    private void loadData() {
        ...
        content.setText(dataReadings.get(position-45).getTopiccontent());
        imageView1.setText(dataReadings.get(position-45).getA());
        imageView2.setText(dataReadings.get(position-45).getB());
        imageView3.setText(dataReadings.get(position-45).getC());
        imageView4.setText(dataReadings.get(position-45).getD());

        answer.setText(TextUtils.isEmpty(Util.strings.get(position))?"":"Selected: "+Util.strings.get(position));

        question.setText(dataReadings.get(position-45).getQuestion());
    }
